...
object Interaction {
  ...
  def tiledev(temperatures: Iterable[(Location, Temperature)], basetemperatures: Iterable[(Location, Temperature)], colors: Iterable[(Temperature, Color)], tile: Tile): Image = {
    val offX = (tile.x * pow(2, power)).toInt
    val offY = (tile.y * pow(2, power)).toInt
    val offZ = tile.zoom
    val coords = for {
    ...
    val basetemp = coords.par
      .map({case (y, x) => Tile(x + offX, y + offY, power + offZ)})
      .map(tileLocation)
      .map(predictTemperature(basetemperatures, _))

    val devtemp = temp.zip(basetemp).map(x => x._1 - x._2)
  ...
  def generateTilesDev[Data](
                           yearlyData: Iterable[(Year, Data, Data)],
    ...
    val tiles = for {
      ...
      y <- 0 until pow(2, zoom).toInt
      yearData <- yearlyData
    } yield generateImage(yearData._1, Tile(x, y, zoom), yearData._2, yearData._3)
  }
