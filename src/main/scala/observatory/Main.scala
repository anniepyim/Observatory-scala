...
def generateAndSaveTileDev(year: Year, tile: Tile, data: Iterable[(Location, Temperature)], basedata: Iterable[(Location, Temperature)]): Unit = {
  ...
  var a = 0
  for( a <- 1995 to 2015){
    val temps = Extraction.locateTemperatures(a, "/stations.csv", "/"+a+".csv")
    val tempsAvg = Extraction.locationYearlyAverageRecords(temps)

    val data = List[(Year, Iterable[(Location, Temperature)], Iterable[(Location, Temperature)])]((a, tempsAvg, basetempsAvg))
